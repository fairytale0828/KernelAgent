...
def aggregate_single_compute_graph(
    ...
    flash_attention_fusions = _find_flash_attention_pattern(subgraphs)
    ...
    mlp_fusions = _find_mlp_pattern(subgraphs, used_indices)
...
def _find_flash_attention_pattern(
    ...
    for i in range(len(subgraphs) - 2):
        ...
        if type1 == "matmul" and type2 == "softmax" and type3 == "matmul":
            # 检查操作序列
            ...
            # 验证是否是 attention 模式
            has_matmul1 = "matmul" in ops1
            has_softmax = any(op in ops2 for op in ["softmax", "online_softmax"])
            has_matmul2 = "matmul" in ops3
            
            if has_matmul1 and has_softmax and has_matmul2:
                # 创建融合子图
                fused_sg = {
                    "id": f"fused_flash_attention_{i}",
                    "type": "flash_attention",
                    "ops": [{"op": "flash_attention"}],
                    "input_shape": sg1.get("input_shape") or sg1.get("inputs", [[]])[0],
...
def _find_mlp_pattern(
    ...
    for i in range(len(subgraphs) - 2):
        ...
        # 检查是否是 MLP 模式
        is_linear1 = type1 in ("linear", "matmul")
        is_activation = type2 in ("activation", "gelu", "relu", "silu")
        is_linear2 = type3 in ("linear", "matmul")
        ...
        if is_linear1 and is_activation and is_linear2:
            ...
            for op in ["gelu", "relu", "silu", "sigmoid"]:
                if op in ops2:
                    activation_type = op
                    break
...
def aggregate_all_compute_graphs(
    ...
    for graph_info in manifest.get("graphs", []):
        ...
        # 聚合
        aggregated_subgraphs, agg_stats = aggregate_single_compute_graph(compute_graph)
        ...
        for fusion in agg_stats.get("fusion_details", []):
            ...
            stats["fusion_patterns"][pattern] = stats["fusion_patterns"].get(pattern, 0) + 1
