...
class PatternParser:
    ...
    def _parse_var(self) -> VarPattern:
        ...
        if not name:
            raise ValueError(f"Expected variable name after '?' at position {self.pos}")
    ...
    def _parse_op_args(self, op_name: str) -> OpPattern:
        ...
        while True:
            ...
            if self.pos >= len(self.text):
                raise ValueError("Unexpected end of pattern, expected ')' or ','")
            ...
            if self.text[self.pos] == ')':
                ...
                raise ValueError(f"Expected ')' or ',' at position {self.pos}, got '{self.text[self.pos]}'")
    ...
    def apply_rewrite(
        ...
        if isinstance(pattern, VarPattern):
            ...
            if pattern.name not in bindings:
                raise ValueError(f"Unbound variable in rewrite: ?{pattern.name}")
            ...
            raise ValueError(f"Cannot apply rewrite for pattern type: {type(pattern)}")
