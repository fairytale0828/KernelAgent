...
class RuleCompiler:
    ...
    def compile(self, rule: Rule) -> PatternRewriteRule:
        ...
        condition = None
        if rule.condition:
            condition = lambda eg, bindings, cond=rule.condition: cond.check(bindings)
    ...
    def _apply_builtin_rules(self, egraph: EGraph, ir: SubgraphIR) -> None:
        ...
        for eclass in list(egraph.iter_eclasses()):
            for enode in list(eclass.nodes):
                ...
                if enode.op == "matmul":
                    variant = self._create_rmsnorm_matmul_variant(egraph, eclass.id, enode)
                    if variant:
                        # 添加变体到同一 eclass
                ...
                # MatMul + Bias 融合
                if enode.op == "add":
                    variant = self._create_matmul_bias_variant(egraph, eclass.id, enode)
                    if variant:
                        eclass.nodes.add(variant)
    ...
    def _apply_associativity(
        ...
        for left_enode in left_eclass.nodes:
            if left_enode.op == enode.op and len(left_enode.children) == 2:
                ...
                # 创建 a op (b op c)
                new_expr = ENode(op=enode.op, children=(a, bc_id), attrs=enode.attrs)
                            ...
                            if child_enode.op == "rsqrt":
                                ...
                                return ENode(
                                    ...
                                    attrs=(
                                        ...
                                        ("original_pattern", "matmul(mul(x, rsqrt), W)"),
                                        ("optimized_pattern", "mul(matmul(x, W), rsqrt)"),
                                    ),
                                )
            ...
            try:
                ...
                for variant_ir, expr in variants:
                    ...
                    for transform in variant_ir.metadata.get("transforms", []):
                        transform_type = transform.split(":")[0] if ":" in transform else "unknown"
                        stats["by_transform"][transform_type] = \
                            stats["by_transform"].get(transform_type, 0) + 1
                
            except Exception as e:
                print(f"Warning: Failed to process subgraph {item.get('id')}: {e}")
                # 保留原始
                if self.config.include_original:
