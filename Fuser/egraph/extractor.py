...
class Extractor:
    ...
    def _backtrack(
        ...
        # 记录选择
        proof.append(f"eclass_{eclass_id}: {enode.op}({', '.join(f'eclass_{c}' for c in enode.children)})")
    ...
    def _extract_with_root(
        ...
        for child_id in root_node.children:
            ...
            if child_id in self._best_node:
                self._backtrack(child_id, choices, proof)
                child_costs.append(self._best_cost.get(child_id, Cost.infinity()))
            else:
                return None
            ...
            if is_complete:
                ...
                for eclass_id in missing:
                    ...
                    for enode in ec.nodes:
                        ...
                        heapq.heappush(heap, (est, counter, new_choices, new_proof))
                        counter += 1
