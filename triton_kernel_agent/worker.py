...
class VerificationWorker:
    ...
    def _call_llm(self, messages: list, **kwargs) -> str:
        ...
        if not self.provider:
            raise RuntimeError(
                f"No provider available for model {self.openai_model}")
        ...
        if self.provider:
            try:
                ...
                if refined_kernel:
                    ...
                    self.logger.error(
                        "Failed to extract valid code from LLM response")
    ...
    def run(
        ...
        for round_num in range(self.max_rounds):
            ...
            if violation:
                ...
                self._log_round(round_num + 1, False,
                                current_kernel, "", message)
                error_info = {
                    "stdout": "",
                    "stderr": message,
        ...
        if best_success_round is not None:
            ...
            self.logger.warning(
                f"Completed all {self.max_rounds} rounds without success")
            return {
                "worker_id": self.worker_id,
                "success": False,
